...
/**
 * The three price tiers, unchanged from AfterBI.
 *
 * OT — open trade, the corner shops. MT — modern trade, the supermarket
 * chains. SEP — special, a negotiated rate for a named account. Every product
 * carries all three and a distributor sees exactly one of them, chosen by
 * `UserProfile.distributorCategory`.
 */
export const PRICE_TIERS = ['OT', 'MT', 'SEP'] as const;
...
export interface Distributor {
  ...
  /** Cleared the moment registration completes. Never re-issued. */
  inviteCode?: string;
...
/** The moving line above every home screen. Platform-authored. */
export interface Notice {
  id: string;
  title: string;
  body: string;
  /** ISO date. Past this, the strip stops drawing it. */
  until: string;
  tone: 'info' | 'warning';
  createdAt: string;
}
