    ...
    },
    {
      ...
      cell: (row) => (
        <span className="text-[13px] font-semibold text-primary tabular">
          {row.subscriptionFee ? naira(row.subscriptionFee) : 'Not set'}
        </span>
      ),
    },
    {
  ...
  if (done) {
    return (
      <Modal
        ...
        description={
          demo
            ? `Sign in as ${done.email} and the demonstration fills itself.`
            : `${done.name} is its super admin. They add everyone else from inside.`
        }
        ...
        <label className="flex items-start gap-2.5 rounded-xl border border-hairline px-3 py-2.5 text-[13px] text-primary">
          ...
          <span>
            ...
            <span className="mt-0.5 block text-[12px] leading-snug text-muted">
              It fills itself with a whole business (catalogue, depots, distributors, six months of trading) the
              first time its administrator signs in, and adds sign-ins for a rep, a depot and a distributor.
            </span>
          </span>
        </label>
