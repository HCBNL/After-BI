  ...
  const columns: Column<Distributor>[] = [
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        return (
          <span className="text-[12.5px] text-muted">{reps.length ? reps.map((r) => r.firstName).join(', ') : 'None'}</span>
        );
      },
    },
    {
    ...
    >
      <div className="space-y-4">
        ...
        */}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="State" hint="Feeds the coverage map on Analytics.">
            <StateSelect value={draft.location ?? ''} onChange={(next) => set('location', next)} />
          </Field>
          <Field
            label="Price tier"
            required
