...
const RemindersPage = lazy(() => import('./pages/shared/RemindersPage'));
 ...
 * THE PUBLIC SITE.
 *
 * `/` used to redirect straight to sign in, which was right when this app had
 * no front of house. It has one now, and these are it: the front page, the
 * product and its eight modules, pricing, about, the walkthrough booking and
 * the two help screens that catch somebody who cannot get in.
 *
 * All lazy, including the front page, and that is not a mistake to fix. They
 * share one chunk — the shell, the kit and the content are imported by all of
 * them — so a visitor fetches it once on arrival and every other public page
 * is then free. What matters is that a rep opening the app on a depot phone
 * never downloads any of it: the sign-in screen stays eager, and the marketing
 * site is not on the path to it.
 */
...
const PUBLIC_PREFIXES = ['/product', '/pricing', '/about', '/demo', '/help'];

function RouteSkeleton({ path }: { path: string }) {
  if (path.startsWith('/portal')) return <ShellSkeleton />;
  if (path === '/' || PUBLIC_PREFIXES.some((prefix) => path === prefix || path.startsWith(`${prefix}/`))) {
    return <SiteSkeleton />;
  }
...
export default function App() {
  ...
  return (
    ...
    <ErrorBoundary resetOn={location.pathname} home={user ? HOME_FOR_ROLE[user.role] : '/login'}>
      {/*
        Three shapes of waiting, because there are three shapes of page.
        The portal gets its own furniture, the public site gets the dark bar
        and hero it always opens with, and the two auth screens — which are
        eager anyway — fall through to the plain one.
      */}
      <Suspense fallback={<RouteSkeleton path={location.pathname} />}>
        <Routes>
          {/*
            ...
            The front page is the front page for everybody, signed in or not.
            It used to bounce straight to a portal, which meant a rep who
            followed a link to afterbi.com to show a distributor what the
            software was could not get to it, and a customer who wanted to read
            the pricing had to sign out first. The bar's own button is what
            adapts instead: it says "Your portal" to somebody who has one.
          */}
          ...
          {/* Somebody already signed in has no use for a reset screen, and
              landing on one is usually a stale link or a shared device. */}
          <Route
          ...
          >
            ...
            <Route path="territories" element={<AdminOnly><AnalyticsPage /></AdminOnly>} />
 ...
 */}
          ...
          {/* Out here a visitor may have no account at all, so the useful 404
              is the public one: it keeps the site's own bar and offers the
              three places they were probably trying to reach. The portal's
              wildcards still use the shared one, which keeps the shell. */}
